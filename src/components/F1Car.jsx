...
function WheelAssembly({ position, isRear, isLeft, wheelRefs, index }) {
  ...
  const wheelRadius = 0.365; // 18-inch regulation
    ...
    <group position={position}>
      <group ref={(el) => (wheelRefs.current[index] = el)}>
        <group rotation={[0, 0, Math.PI / 2]}>
          ...
          {/* Red Sidewall Stripe (Soft Compound) */}
...
function F1Car({ mouse }) {
  ...
  useFrame((state, delta) => {
    ...
    const currentX = (LOOP_RANGE / 2) - ((time * CAR_SPEED) % LOOP_RANGE);
    groupRef.current.position.x = currentX;

    const targetY = mouse.current[1] * -2.5;
    groupRef.current.position.y = THREE.MathUtils.lerp(groupRef.current.position.y, targetY, 0.08);
    
    groupRef.current.position.y += Math.sin(time * 45) * 0.005;
    groupRef.current.rotation.x = Math.sin(time * 9) * 0.003;
    const lean = (targetY - groupRef.current.position.y);
    groupRef.current.rotation.z = THREE.MathUtils.lerp(groupRef.current.rotation.z, lean * 0.18, 0.08);
    
    const rotationSpeed = (CAR_SPEED / 0.365) * delta;
  ...
  return (
    <group ref={groupRef} scale={1.7} rotation={[0, -Math.PI / 2, 0]}>
      {/* Sharp Nose Cone */}
      ...
      {/* Sculpted Sidepods with Deep Undercut */}
      ...
      {/* Venturi Channels (Ground Effect) */}
...
export default function HyperRealisticF1() {
  const mouse = useRef([0, 0]);

  return (
    // Changed: Absolute positioning, transparent background, proper Z-index
    <div className="absolute inset-0 z-0 w-full h-full">
      ...
      >
        <PerspectiveCamera makeDefault position={[0, 2.2, 11]} fov={42} />
        ...
        {/* REMOVED: The Ground Plane Mesh that was blocking the background */}
      ...
      {/* REMOVED: The F1 2025 Text Overlay */}
    </div>
  );
}